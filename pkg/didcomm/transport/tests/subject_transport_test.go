...
func TestSubjectOutboundTransport_SendAndReceive(t *testing.T) {
	...
	// Register handler
	transport.RegisterSubjectEndpoint(endpoint, func(payload []byte) (int, []byte, string) {
	...
	defer transport.UnregisterSubjectEndpoint(endpoint)

	// Create a dummy encrypted message
	msg := &envelopeServices.EncryptedMessage{Protected: "hdr", IV: "iv", Ciphertext: "ct", Tag: "tag"}

	// Send
	tr := transport.NewSubjectOutboundTransport()
	if !tr.CanSend(endpoint) {
		t.Fatalf("transport should be able to send to %s", endpoint)
	}
	...
	if len(body) != 0 || ctype != "" {
		t.Fatalf("unexpected response: body=%v ctype=%s", body, ctype)
	}

	// Verify handler received payload
	select {
	case got := <-received:
		if got.Ciphertext != msg.Ciphertext || got.Tag != msg.Tag {
