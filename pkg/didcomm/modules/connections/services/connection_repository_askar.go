...
func (r *StorageConnectionRepository) Save(ctx *agentcontext.AgentContext, record *ConnectionRecord) error {
	...
	if record.BaseRecord == nil {
		record.BaseRecord = storage.NewBaseRecord("ConnectionRecord")
	} else {
		record.BaseRecord.Type = "ConnectionRecord"
	}
	if record.Tags == nil {
	...
	if record.Did != "" {
		record.Tags["did"] = record.Did
	}
	if record.TheirDid != "" {
		record.Tags["theirDid"] = record.TheirDid
	}
	if record.InvitationKey != "" {
		record.Tags["invitationKey"] = record.InvitationKey
	}
	if record.TheirRecipientKey != "" {
		record.Tags["theirRecipientKey"] = record.TheirRecipientKey
	}
	if record.OutOfBandId != "" {
		record.Tags["outOfBandId"] = record.OutOfBandId
	}
...
func (r *StorageConnectionRepository) cast(rec storage.Record) (*ConnectionRecord, error) {
	if cr, ok := rec.(*ConnectionRecord); ok {
		return cr, nil
	}
