...
func (m *ConnectionRequestMessage) Validate() error {
	...
	if m.Label == "" {
		return fmt.Errorf("connection request must have a label")
	}
	if m.Connection == nil {
		return fmt.Errorf("connection request must have connection info")
	}
	if m.Connection.Did == "" {
		return fmt.Errorf("connection request must have a DID")
	}
	...
	if m.ImageUrl != "" && !utils.IsValidURL(m.ImageUrl) {
...
func (m *ConnectionRequestMessage) CreateReplyMessage(messageType string) *messages.BaseMessage {
	return m.BaseMessage.CreateReplyMessage(messageType)
}

// IsThreadedReplyTo checks if this message is a threaded reply to another message
func (m *ConnectionRequestMessage) IsThreadedReplyTo(originalMessage messages.MessageInterface) bool {
	return m.BaseMessage.IsThreadedReplyTo(originalMessage)
}
...
func (m *ConnectionRequestMessage) ToLegacyFormat() *ConnectionRequestMessage {
	clone := m.Clone().(*ConnectionRequestMessage)
	clone.SetType(ConnectionRequestTypeV1_0)
	return clone
}
func (m *ConnectionRequestMessage) IsLegacyFormat() bool {
	return m.GetType() == ConnectionRequestTypeV1_0
}
...
func (m *ConnectionRequestMessage) ExtractServiceEndpoint() (string, error) {
	if m.Connection == nil || m.Connection.DidDoc == nil {
		return "", fmt.Errorf("no DID document available")
	}
...
func (m *ConnectionRequestMessage) ExtractRecipientKeys() ([]string, error) {
	...
	if len(keys) == 0 {
		return nil, fmt.Errorf("no recipient keys found in DID document")
	}
	return keys, nil
}
...
func (m *ConnectionRequestMessage) ValidateAgainstInvitation(invitation *ConnectionInvitationMessage) error {
	...
	if !m.IsThreadedReplyTo(invitation) {
		return fmt.Errorf("request is not a threaded reply to the invitation")
	}
