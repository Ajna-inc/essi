...
func (r *AskarHolderCredentialRepository) Update(ctx *agentcontext.AgentContext, record *CredentialRecord) error {
	...
	if cast, ok := existing.(*HolderCredentialRecord); ok {
		rec = *cast
	} else {
		b, _ := existing.ToJSON()
...
func (r *AskarHolderCredentialRepository) GetByFilter(ctx *agentcontext.AgentContext, filter *svc.CredentialFilter) ([]*CredentialRecord, error) {
	...
	if filter != nil {
		if filter.SchemaId != "" {
			q.WithTag("schemaId", filter.SchemaId)
		}
		if filter.CredentialDefinitionId != "" {
			q.WithTag("credentialDefinitionId", filter.CredentialDefinitionId)
		}
	}
	...
	for _, rec := range list {
		if cr, err := r.toCredentialRecord(rec); err == nil {
			out = append(out, cr)
		}
	}
	return out, nil
...
func (r *AskarHolderCredentialRepository) toCredentialRecord(rec storage.Record) (*CredentialRecord, error) {
	...
	if cast, ok := rec.(*HolderCredentialRecord); ok {
		tmp = *cast
	} else {
		b, _ := rec.ToJSON()
